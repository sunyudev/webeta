...
<body>
	<form class="form-inline definewidth m20" action="user/vague.do"
		method="get">
	...
	<table class="table table-bordered table-hover definewidth m10">
		...
		<c:forEach items="${projectList.list }" var="p" varStatus="status">
			<tr>
				...
				<td><a href="user/toEditPage.do?name=${p.userName }&pageNum=${projectList.pageNum}">编辑</a>&nbsp;&nbsp;&nbsp;
		...
		<tr>
			<th colspan="5">
				<div class="inline pull-right page">
					...
					<c:if test="${!projectList.isFirstPage }">
						<a href='user/forward.do?pageNum=${projectList.pageNum - 1 }'>&laquo;</a>
					</c:if>
					<c:forEach items="${projectList.navigatepageNums }" var="n">
						<!-- 判断是否是当前页,是导航页就是选中状态 -->
						<c:if test="${n==projectList.pageNum }">
							<a class="product-ym_bg" href="user/forward.do?pageNum=${n }">${n }</a>
						</c:if>
					...
					<c:if test="${!projectList.isLastPage }">
						...
						<a class="product-ym_xyy"
							href="user/forward.do?pageNum=${projectList.pageNum + 1 }"
...
/* 物理删除用户 */
	function del(id) {
		...
		if (truthBeTold) {
			 window.location.href="user/del.do?id="+id
